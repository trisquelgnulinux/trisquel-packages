...
class AppInstallApp(SimpleGtkbuilderApp):
    ...
    def tryInstall(self, item):
        ...
        group = apt_pkg.GetPkgActionGroup(self.cache._depcache)
        # reapply the state of installs
        (to_add, to_rm) = self.menu.getChanges()
        for app in to_add:
            self.cache[app.pkgname].markInstall()
        del group
        ...
        # existing one
        if (self.cache._depcache.DelCount > 0 and
            self.cache._depcache.BrokenCount == 0 and
            not apt_error):
            if not hasattr(item, "replaces"):
                return self.canNotInstallApp(pkg)
            item_replaces = set(item.replaces)
            removals = set([pkg.name for pkg in self.cache.getChanges() if pkg.markedDelete])
        ...
        if apt_error or self.cache._depcache.BrokenCount > 0:
            return self.canNotInstallApp(pkg)
        return True
    ...
    def on_install_toggle(self, widget, item):
        ...
        is_installed = self.cache.has_key(pkg) and self.cache[pkg].isInstalled
        ...
        try:
            if not hasattr(self, "cache"):
                ...
                if self.activation_style.isSpecific():
                    ...
                    self.cache = None
                    #now = time.time()
                    loader = ThreadedCache(self, apt.progress.OpProgress())
